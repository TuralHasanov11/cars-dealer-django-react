...
export default function EditForm({car, brands, carModels, gearLevers, years, carBodies, fuels, engines, transmissions, colors, cities, equipment }){
    ...
    } = useInput({validations:validations, initialState:car.made_at, rules:{required:true,isInteger:true,min:1980, max:new Date().getFullYear()}})
    ...
    } = useInput({validations:validations, initialState:car.price, rules:{required:true,isInteger:true,min:200}})
    ...
    } = useInput({validations:validations, initialState:car.description, rules:{nullable:true}})
    ...
    const [carEquipment, setCarEquipment] = useState(car.equipment.map(el => el.id))
            ...
            <div className="border-top border-light mt-2 pt-4">
                ...
                <div className="col-md-6">
                    ...
                    <select onChange={onCarBodyChange} defaultValue={carBody} className={`form-select form-select-light mb-3 ${!carBodyIsValid?'is-invalid':'is-valid'}`} id="car_body">
                        ...
                        {carBodies.map((body, index)=>(<option key={index} value={body.id}>{body.name}</option>))}
                    </select>
                    ...
                    <select onChange={onFuelChange} defaultValue={fuel} className={`form-select form-select-light mb-3 ${!fuelIsValid?'is-invalid':'is-valid'}`} id="fuel">
                        ...
                        {fuels.map((fuel, index)=>(<option key={index} value={fuel.id}>{fuel.name}</option>))}
                    </select>
                    ...
                    <select onChange={onEngineChange} defaultValue={engine} className={`form-select form-select-light mb-3 ${!engineIsValid?'is-invalid':'is-valid'}`} id="engine">
                        ...
                        {engines.map((engine, index)=>(<option key={index} value={engine.id}>{engine.volume}</option>))}
                    </select>
                    ...
                    <select onChange={onTransmissionChange} defaultValue={transmission} className={`form-select form-select-light mb-3 ${!transmissionIsValid?'is-invalid':'is-valid'}`} id="transmission">
                        ...
                        {transmissions.map((transmission, index)=>(<option key={index} value={transmission.id}>{transmission.name}</option>))}
                    </select>
                    ...
                    <select onChange={onGearLeverChange} defaultValue={gearLever} className={`form-select form-select-light mb-3 ${!gearLeverIsValid?'is-invalid':'is-valid'}`} id="gear_lever">
                        ...
                        {gearLevers.map((lever, index)=>(<option key={index} value={lever.id}>{lever.name}</option>))}
                    </select>
                    ...
                    <div key={index} className="col-sm-4">
                        <div className="form-check form-check-light">
                            <input 
                                ...
                                type="checkbox" 
                                checked={carEquipment.includes(item.id)} 
                                value={item.id} 
